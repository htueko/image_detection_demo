...
object PermissionObject {
    ...
    const val REQUEST_PERMISSION_CODE = 1201
    ...
    private fun toRequestPermissions(
        ...
        if (ActivityCompat.shouldShowRequestPermissionRationale(activity, permissions[0])) {
            ...
            AlertDialog.Builder(activity)
                ...
                .setMessage(message)
                .setPositiveButton(
                    activity.resources.getString(R.string.label_ok)
                ) { dialog, which ->
                    // show explanation then request
                    ActivityCompat.requestPermissions(activity, permissions, code)
                }
                .setNegativeButton(activity.resources.getString(R.string.label_cancel)) { dialog, which ->
                    dialog.dismiss()
                }.create().show()
